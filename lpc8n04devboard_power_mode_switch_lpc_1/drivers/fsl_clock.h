...
#define CLK_GATE_GET_REG(x) (((uint32_t)(x) >> 24U) & 0xFFU)
...
{
    *(volatile uint32_t *)(((uint32_t)(&SYSCON->SYSAHBCLKCTRL)) + CLK_GATE_GET_REG(clk)) |=
        CLK_GATE_GET_VALUE_SHIFT(clk);
}
...
static inline uint32_t CLOCK_GetSysClkFreq(void)
{
    ...
    sysclkctrl = (SYSCON->SYSCLKCTRL) >> 1U;
    return g_Sfro_Osc_Freq / (1UL << ((sysclkctrl)&0x7U));
}
