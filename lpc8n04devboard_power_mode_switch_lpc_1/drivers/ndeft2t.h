...
#include "ndeft2t_dft.h"

/** @defgroup MODS_LPC8Nxx_NDEFT2T ndeft2t: NDEF Type-2 Tag Message creator/parser Module
 ...
 *          This has to be done after initialising the NFC HW block (see #NFC_Init) by calling #NDEFT2T_Init.
 *          Similarly, the mod can be de-initialised by calling #NDEFT2T_DeInit followed by de-initialisation of the
 *          NFC HW block (see #NFC_DeInit) after the application processing is complete. Note that the
 ...
 *             #NDEFT2T_CreateExtRecord or #NDEFT2T_CreateMimeRecord for creating NFC Forum well known types TEXT and
 * URI,
 ...
 *  #NFC_ClearInterruptStatus. This MOD provides 2 callback functions of types defined by #pNdeft2t_FieldStatus_Cb_t
 ...
 *  #kNFC_RFSelectInterrupt, #kNFC_TargetWriteInterrupt and #kNFC_NFCOffInterrupt interrupts. However,
 * #kNFC_MemWriteInterrupt and
 *  #kNFC_TargetReadInterrupt interrupts are also enabled/disabled by the MOD internally during NDEF message creation.
 ...
 *
 * @note The #kNFC_TargetWriteInterrupt and #kNFC_TargetReadInterrupt interrupts are internally configured by the mod to
 * occur for
 ...
 *  shared memory for the record payload.
 *      .
 *
 * @par Diversity
 ...
 *  Check @ref MODS_LPC8Nxx_NDEFT2T_DFT for all diversity parameters.
 *
 *
 * @{
 */
 ...
 *  payload.
 */
#define NDEFT2T_MSG_OVERHEAD(shortMessage, totalRecordOverhead) \
    (8 + 2 + ((shortMessage) ? 1 : 3) + (totalRecordOverhead))
...
#define NFC_SHARED_MEM_BYTE_SIZE (sizeof(((RFIDNFC_Type *)0)->SHARED_MEM)) /*!< NFC shared RAM size in bytes. */
 ...
 * @pre: Initialise NFC HW block (see #NFC_Init)
 */
void NDEFT2T_Init(RFIDNFC_Type *base);

/**
 * This function De-initialises the NDEFT2T module.
 * @post: De-initialisation of NFC HW block (see #NFC_DeInit) is now possible
 */
 ...
 *             is set to 0 and if the argument @c shortMessage in #NDEFT2T_CreateMessage API was set wrongly by the
 * caller.
 *  .
 */
...
/**
 * This function is NDEFT2T implementation of NFC IRQ handler and it is to be called from the interrupt vector
 * #NFC_IRQHandler. Handler calls user defined callback functions #pNdeft2t_FieldStatus_Cb_t and
 * #pNdeft2t_MsgAvailable_Cb_t, if enabled.
 * @param pInstance : Base address of instance Buffer
 */
