...
#define NDEFT2T_NDEF_PAYLOAD_START_OFFSET_SHORT                                            \
    NDEFT2T_NDEF_TLV_START_OFFSET + 0x2 /*!< Offset from the shared memory start   \ \ \ \ \
...
#define NDEFT2T_NDEF_PAYLOAD_START_OFFSET_LONG                                               \
    NDEFT2T_NDEF_TLV_START_OFFSET + 0x4 /*!< Offset from the shared memory start     \ \ \ \ \
    where the actual payload or V field starts for a message of length >= 255 bytes. \ \ \ \ \
    This includes the Lock TLV and the Proprietary TLV lengths as well. */
#define NDEFT2T_MIN_RECORD_HEADER_FIXED_LENGTH                                   \
    0x04 /*!< Minimum length of the fixed part of an NDEF record header. \ \ \ \ \
    ...
    assumed here. */
#define NDEFT2T_MAX_RECORD_HEADER_FIXED_LENGTH                                                                        \
    0x07                                      /*!< Maximum length of the fixed part of an NDEF record header. \ \ \ \ \
                                         These fields are the record header byte, TYPE LENGTH, PAYLOAD LENGTH \ \ \ \ \
...
#define NDEFT2T_NDEF_SHORT_MSG_LIMIT                                                        \
    0xFE /*!< If message size is less than or equal to oxFE, then a one byte length \ \ \ \ \
...
#define NDEFT2T_EMPTY_NDEF_MSG_HDR                                                              \
    0x00000003 /*!< Empty NDEF Message Header, with first 2 bytes reserved as NULL TLVs \ \ \ \ \
...
static const uint8_t __attribute__((aligned(4))) defaultBytes[] = {NDEFT2T_TLV_PROPRIETARY,
                                                                   0x02,
                                                                   0x00,
                                                                   0x00,
...
static bool CreateRecord(void *pInstance,
                         const NDEFT2T_CREATE_RECORD_INFO_T *pRecordInfo,
                         NDEFT2T_RECORD_TYPE_T type,
                         NDEFT2T_TNF_T tnf,
                         int hdrLen,
                         bool typeStringPresent);
static uint8_t *DecodeNdefTlv(RFIDNFC_Type *base, int *lenTlv);
static bool ValidateNdefMsg(void *pInstance);
static void EnableTermTlvDetection(RFIDNFC_Type *base);
static void DisableTermTlvDetection(RFIDNFC_Type *base);
#if NDEFT2T_COLLISION_DETECTION == 1
static bool NFC_WordWrite(RFIDNFC_Type *base, uint32_t *pDest, const uint32_t *pSrc, int n);
static bool NFC_ByteRead(RFIDNFC_Type *base, uint32_t *pDest, const uint32_t *pSrc, int n);
#endif

static volatile uint32_t sTermTlvOffset; /** Holds the byte offRFIDNFC_Type *base, set location of the terminator TLV in
                                            the message that is getting parsed. */
static volatile uint32_t
    sTermTlvPage; /** Holds the 32 bit word from the message getting created corresponding to the
...
void NDEFT2T_Init(RFIDNFC_Type *base)
{
    ...
    NFC_EnableInterrupts(base, kNFC_RFSelectInterrupt | kNFC_TargetWriteInterrupt | kNFC_NFCOffInterrupt);
    NVIC_EnableIRQ(NFC_IRQn);
    stickyMEM_WRITE |= (base->RIS & kNFC_MemWriteInterrupt);
    NFC_ClearInterruptStatus(base, kNFC_AllInterrupt);
}

/** De-Initialises NDEFT2T MOD. */
void NDEFT2T_DeInit(RFIDNFC_Type *base)
{
...
bool NDEFT2T_CommitMessage(RFIDNFC_Type *base, void *pInstance)
{
    NDEFT2T_INSTANCE_T *pInst = (NDEFT2T_INSTANCE_T *)pInstance;
    uint32_t *pCursor;
    uint32_t *pMem;
    ...
    *pInst->pCursor++ = NDEFT2T_TLV_TERMINATOR; /* Write Terminator TLV. */
    ...
    do
    {
        statusPayload = NFC_WordWrite(base, pMem, pCursor, msgSize / 4);
        tries++;
    ...
    {
        ndefHdr =
            (int)((lenTlv << 24) | ((lenTlv << 8) & 0xFF0000) | (NDEFT2T_NDEF_3BYTE_LEN_START << 8) | NDEFT2T_TLV_NDEF);
    }
    else
    {
        ndefHdr =
            *((int *)((int)base->SHARED_MEM + NDEFT2T_NDEF_TLV_START_OFFSET)); /*Retrieve the NDEF message header. */
        ndefHdr &= (int)0xFFFF00FF; /* Clear length field, which is at at byte position 1. This might or might not be
                                    '0' depending on initial setting of pInst->shortMessage*/
        ndefHdr |= lenTlv << 8;     /* Fill length. */
    }
#if NDEFT2T_COLLISION_DETECTION == 0
    *((int *)((int)base->SHARED_MEM + NDEFT2T_NDEF_TLV_START_OFFSET)) = ndefHdr;
#else
    tries = 0;
    do
    {
        statusHdr = NFC_WordWrite(base, (uint32_t *)((int)base->SHARED_MEM + NDEFT2T_NDEF_TLV_START_OFFSET),
                                  (uint32_t *)&ndefHdr, 1);
        tries++;
...
bool NDEFT2T_GetMessage(RFIDNFC_Type *base, void *pInstance, uint8_t *pBuffer, int bufLen)
{
    NDEFT2T_INSTANCE_T *pInst = (NDEFT2T_INSTANCE_T *)pInstance;
    uint8_t *pV;
    int lenTlv;
    ...
    if ((uint32_t)(pV + lenTlv) > ((uint32_t)base->SHARED_MEM + NFC_SHARED_MEM_BYTE_SIZE - 1))
    {
        return false;
    }
    ...
    if (lenTlv)
    {
        ...
        do
        {
            status = NFC_ByteRead(base, (uint32_t *)pBuffer, (uint32_t *)pV, lenTlv);
            i++;
    ...
    switch (tnf)
    {
        ...
        case NDEFT2T_TNF_NFC_RTD:
            ...
            if (typeLen == 0x01)
            {
                ...
                if (type == NDEFT2T_NFC_RTD_TEXT)
                { /* Check for TEXT type record. */
                ...
                else if (type == NDEFT2T_NFC_RTD_URI)
                { /* Check for URI type record. */
 ...
 *                       clear shortRecord based on the need to use short records. The pString field is not applicable
 * for
...
static uint8_t *DecodeNdefTlv(RFIDNFC_Type *base, int *lenTlv)
{
    uint8_t *pMem     = ((uint8_t *)base->SHARED_MEM) + NDEFT2T_NDEF_TLV_START_OFFSET;
...
static void EnableTermTlvDetection(RFIDNFC_Type *base)
{
    int mask = NFC_GetEnabledInterrupts(base);
    NFC_EnableInterrupts(base, (uint32_t)mask | kNFC_MemWriteInterrupt | kNFC_TargetReadInterrupt);
}
...
{
    int mask = NFC_GetEnabledInterrupts(base);
    NFC_EnableInterrupts(base, (uint32_t)mask & (~(uint32_t)(kNFC_MemWriteInterrupt | kNFC_TargetReadInterrupt)));
    sTermTlvOffset = NDEFT2T_TERM_TLV_INIT_VAL;
}
/**
...
static bool NFC_WordWrite(RFIDNFC_Type *base, uint32_t *pDest, const uint32_t *pSrc, int n)
{
    return NFC_ByteRead(base, pDest, pSrc, n * 4);
}
static bool NFC_ByteRead(RFIDNFC_Type *base, uint32_t *pDest, const uint32_t *pSrc, int n)
{
    stickyMEM_WRITE = 0;
    ...
    base->ICR = kNFC_MemWriteInterrupt;

    memcpy(pDest, pSrc, (uint32_t)n);

    if (((base->RIS & kNFC_MemWriteInterrupt) == kNFC_MemWriteInterrupt) || stickyMEM_WRITE)
    {
        return false;
    }
    return true;
...
void NDEFT2T_IRQHandler(RFIDNFC_Type *base)
{
    ...
    int nfcInterruptStatus       = NFC_GetInterruptStatus(base);
    int nfcInterruptMaskedStatus = (int)(base->MIS & kNFC_AllInterrupt);
    stickyMEM_WRITE |= (base->RIS & kNFC_MemWriteInterrupt);
    NFC_ClearInterruptStatus(base, nfcInterruptStatus);
    ...
    if (nfcInterruptMaskedStatus & kNFC_TargetWriteInterrupt)
    {
        DisableTermTlvDetection(base);
        uint32_t firstBytes = *(base->SHARED_MEM + 2);
        nfcInterruptMaskedStatus &= ~kNFC_MemWriteInterrupt; /* kNFC_MemWriteInterrupt can also be set at this point, if
        ...
        { /* Check, if we have a valid NDEF message detected?. A non-empty NDEF
message will have the byte after the NDEF TLV (which is 0x03) set to a non-zero value. */
            ...
            if (pV != NULL)
            {
                sTermTlvOffset = (uint32_t)((uint32_t)pV - (uint32_t)base->SHARED_MEM + (uint32_t)lenTlv);
#if defined(NDEFT2T_MSG_AVAILABLE_CB)
     ...
     * based on the occurrence of #kNFC_MemWriteInterrupt interrupt during/after the writing of the created message to
     * shared
    ...
    if ((nfcInterruptMaskedStatus & kNFC_MemWriteInterrupt) && (sTermTlvOffset != NDEFT2T_TERM_TLV_INIT_VAL))
    {
        /* Corruption detected, apply correction. */
        *(base->SHARED_MEM + (sTermTlvOffset / 4)) = sTermTlvPage;
    }

    /* Terminator TLV detection and correction logic is disabled on getting one of #kNFC_NFCOffInterrupt,
     * #kNFC_RFSelectInterrupt
     * or #kNFC_TargetReadInterrupt. The first one indicates the end of the current read or write operation from the RF
     * reader
     * and the remaining indicates the start of a new operation. */
    if ((nfcInterruptMaskedStatus & kNFC_NFCOffInterrupt) || (nfcInterruptMaskedStatus & kNFC_RFSelectInterrupt) ||
        (nfcInterruptMaskedStatus & kNFC_TargetReadInterrupt))
    {
        DisableTermTlvDetection(base);
    }

#if defined(NDEFT2T_FIELD_STATUS_CB)
    /* Translate the kNFC_NFCOffInterrupt (Indicating Field OFF) and kNFC_RFSelectInterrupt (Indicating Field ON)
     * interrupts to a
    ...
    if (nfcInterruptMaskedStatus & kNFC_RFSelectInterrupt)
    {
        if (nfcInterruptMaskedStatus & kNFC_NFCOffInterrupt)
        {
            NDEFT2T_FIELD_STATUS_CB((NFC_GetStatusFlag(base) & kNFC_SelFlag) != 0);
        }
        else
        {
    ...
    else if (nfcInterruptMaskedStatus & kNFC_NFCOffInterrupt)
    {
